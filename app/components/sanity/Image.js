...
const sanityImageLoader = ({ src, width, quality = 75 }) => {
  return `${src}${src.includes("?") ? "&" : "?"}w=${width}&q=${quality}&auto=format`;
};
  ...
  return (
    <div className="w-100 h-auto flex justify-center rounded-xl">
      <figure className="w-full h-auto rounded-xl">
        ...
        />
        <figcaption className="mt-2 text-sm text-center text-gray-500">{value.caption}</figcaption>
      </figure>
    </div>
  );
}

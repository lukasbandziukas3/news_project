...
const LandingNewsSection = ({ blogs }: { blogs: BlogPost[] }) => {
  ...
  // Separate latest 3 blogs and the rest as featured blogs
  const latestBlogs = sortedBlogs.slice(0, 3);
  const featuredBlogs = sortedBlogs.slice(3);
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-[80rem]">
        <h2 className="text-2xl sm:text-3xl font-bold text-center text-primary-600 mb-6">
          Latest News
        </h2>
        {isMobile ? (
          <div className="flex flex-col gap-6">
            {latestBlogs.map((item: BlogPost) => (
              <BlogCard
                key={item._id}
                id={item.slug}
                title={item.title}
      ...
      {featuredBlogs.length > 0 && (
        <div className="w-full max-w-[80rem] mt-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {featuredBlogs.map((item: BlogPost) => (
              <BlogCard
                key={item._id}
                id={item.slug}
                title={item.title}
